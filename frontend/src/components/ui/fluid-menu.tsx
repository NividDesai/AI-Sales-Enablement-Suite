  ...
  return (
    <div className="relative w-[64px]" data-expanded={isExpanded}>
      ...
      <div className="relative">
        ...
        <div 
          className="relative w-16 h-16 bg-black/90 dark:bg-black/90 backdrop-blur-sm border border-white/10 cursor-pointer rounded-full group will-change-transform z-50 shadow-2xl"
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-16 h-16 bg-black/90 dark:bg-black/90 backdrop-blur-sm border border-white/10 will-change-transform rounded-full shadow-2xl"
            style={{
              ...
              clipPath: index === childrenArray.length - 2 
                ? "circle(50% at 50% 50%)" 
                : "circle(50% at 50% 55%)",
              transition: `transform ${isExpanded ? '200ms' : '200ms'} cubic-bezier(0.4, 0, 0.2, 1),
                         opacity ${isExpanded ? '200ms' : '250ms'}`,
